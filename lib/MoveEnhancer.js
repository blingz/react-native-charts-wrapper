...
export default function MoveEnhancer(Chart) {
  return class MoveExtended extends Chart {
    ...
    changeZoom(scaleX, scaleY, centerX, centerY, axisDependency) {
      UIManager.dispatchViewManagerCommand(
        findNodeHandle(this.getNativeComponentRef()),
        UIManager[this.getNativeComponentName()].Commands.changeZoom,
        [scaleX, scaleY, centerX, centerY, axisDependency]
      );
    }
  }
}